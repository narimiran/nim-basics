...
All elements in an array, must share the same type, meaning they are homogenous.
Arrays are also of static size, meaning that the amount of elements (or rather the amount of possible elements), must be known on compile-time.
This means that we call arrays for a "homogenous container of constant length".
...
Remember that since `length` here is a constant it must be known on compile-time.
...
Since the length has to be known at compile-time this will not work:
...
Note however that seqs are also homogenous so every element in a seq has to be the same type.
...
Both of the containers we've seen so far have been homogenous, meaning they must contain only values of the same type.
Tuples on the other hand contain heterogenous data, i.e. elements of a tuple can be of different types.
Similarily to arrays, tuples are fixed-size.
