...
We can think of it as various branches, and we pick our path depending on some condition.
For example, we will buy eggs only _if_ their price is less than some value. Or: _if_ it is raining, we will take an umbrella, otherwise (_else_) we will take sunglasses.
...
If statement is the simplest way to branch our program.
...
The `condition` must be of boolean type -- either a boolean variable or relational and/or logical expression.
All lines indented two spaces make the same if-block and will be executed only if the condition is `true`.
...
Case statement is similar to the if statement with multiple ``elif``s.

Code written with if-elif block which looks like this:
...
if x == 5:
  ...
  echo "unknown number"
----
can be written with case statement:
...
We can use multiple values for each branch.
[source]
.multipleCase.nim
----
