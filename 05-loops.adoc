...
Loops are constructs which allow us to run some parts of code repeatedly, either a known number of times (for-loops) or as long as some condition is satisfied (while-loops).
...
If we want to iterate through a range of (integer) numbers, Nim syntax for that is `start .. finish`.
This will iterate through all numbers between `start` and `finish`.
Both ends are included in the iteration, and `start` needs to be smaller than `finish`.

If we want to iterate _until_ a number (not including it), we use `..<`.
...
If we want to iterate through a range of numbers with a step size different than one, `countup` is used, where we define the starting value, the stopping value (included in the range), and the step size.
...
----
<1> To iterate from a higher to a lower number, we must use `countdown`. (Operator `..` can be used only when starting value is smaller than the end value.)
...
We can use for-loop to iterate through each character of a string (this kind of iteration is sometimes called _for-each_ loop).
...
If we also need to have an iteration counter (starting from zero), we can achieve that by using `for <counter>, <loopVariable> in <iterator>:` syntax.
...
With it, when `i` becomes 3, we immediately exit the loop (before printing the value of `i`).
...
1. https://en.wikipedia.org/wiki/Collatz_conjecture[Collatz conjecture] is a popular mathematical problem with the simple rules. Pick a number. If it is odd, multiply it by three and add one; if it is even, divide it by two. Repeat this procedure until you arrive at one. E.g. 5 -> odd -> 3*5 + 1 = 16 -> even -> 16 / 2 = 8 -> even -> 4 -> 2 -> 1 -> end! +
