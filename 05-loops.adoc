...
A loop is another control flow construct which allow us to run some parts of code multiple times. This can either be a known number of times (for-loops) or for as long as some condition is satisfied (while-loops).
...
The `continue` statement starts the next iteration of a loop immediately, without executing the remaining lines of the current iteration. Notice how 3 is missing from the output of the following code:
