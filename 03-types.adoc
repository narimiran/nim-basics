...
NOTE: When using the `int` function to convert a float to an integer no rounding will be performed. The number simply drops any decimals. To perform rounding we must call another function, but for that we must know a bit more about how to use Nim.
...
If we don't want to change the original string we can also concatenate (joined together) strings with the `&` operator, this returns a new string.
...
Here are all the relational operators defined for integers, note that we can also create our own:
...
Types might seem like a restriction at first, but it allows the Nim compiler to both make your code faster, and make sure you're not doing something wrong by accident.
