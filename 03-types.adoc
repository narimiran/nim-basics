...
Notice that we can use `_` as a thousands separator, to make larger numbers more readable (it is easier to see we're talking about 10 million when written like 10_000_000 than like 10000000).

The expected mathematical operators -- addition (`+`), subtraction (`-`), multiplication (`*`), and division (`/`) -- work as one would expected.
The first three operations always produce integers, while dividing two integers always gives floating point number (a number with a decimal point) as a result, even if two numbers can be divided without a remainder.

Integer division (division where the fractional part is discarded) can be used with an operator `div`.
An operator `mod` is used if one is interested in a remainder (modulus) of an integer division.
...
----
<1> `echo` command will print to the screen everything that follows it separated by commas. In this case, it first prints the string ``a + b = ``, and then after it, in the same row, it prints the result of the expression `a + b`.
...
Notice that we can use scientific notation for large floats, where number after `e` is the exponent.
...
Conversion is straight-forward -- to convert to an integer, we use `int` function, and to convert to a float the `float` function is used.
[source]
----
let
  e = 5
...
<2> Printing `int` version of a float `f`.
...
----
<1> An empty string.
<2> This is not a number (int). It inside double quotes, making it a string.
...
Two strings can be concatenated (joined together) with the operator `&`.
...
Do not confuse it with `=`, which is used to assign some value to a variable.

See the following example for all relational operators:
...
Logical operators are used to test the truthiness of an expression consisting of two or more boolean operands.

* logical `and` gives `true` only if both members are `true`
* logical `or` gives `true` if there is at least one member which is `true`
* logical `xor` gives `true` if one member is true, but the other is not 
...
5. Create an immutable variable containing your first name, and another one containing your last name. Make a variable `fullName` by concatenating previous two variables. Don't forget to put a whitespace between. Print your full name.
