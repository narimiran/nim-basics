...
NOTE: As mentioned in the beginning of this section, procedures are often called functions in other languages. +
...
The body of a procedure is written in the indented block following the declaration. Note that to pass a block the procedure declaration must also be appended with a `=` sign.
...
In order for this to work we need to let Nim, and the programmer using our procedure, that we are going to change it by declaring it as a variable:
...
This one is also a call where the first argument is written before the function name, and the rest of the parameters are stated in parentheses:
...
These two can also of course be combined like this:
...
This syntax however is not seen very often.
...
----
<1> Sequences must be _initialized_, deleting this line will cause an error. Notice that `var` must _not_ be used, as `result` is already _implicitly declared_.
...
<3> Third way of calling procedure, as we saw above.
...
<2> This is were `plus` is actually implemented, this must of course match our previous definition.
