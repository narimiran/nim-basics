...
WARNING: Beware!
The default value of strings and sequences is https://en.wikipedia.org/wiki/Null_pointer[`nil`], and when we use them as returning types, the `result` variable needs to be initialized as an empty string (`""`) or as an empty sequence (`@[]`).
